...
const defaultScoreMap = {
  ...
  followedByTalentProtocol: 4,
  followingOnTalentProtocol: 6,
  commonPoaps: 5
};
...
const isBurnedAddress = (address: string) => {
  ...
  return (
    address === "0x0000000000000000000000000000000000000000" || address === "0x000000000000000000000000000000000000dead"
  );
};

const calculatingScore = (
  user: PoapUser | FarcasterFollowerAddress | FarcasterFollowingAddress | LensFollowingAddress | LensFollowerAddress,
  scoreMap = defaultScoreMap
): PoapUser | FarcasterFollowerAddress | FarcasterFollowingAddress | LensFollowingAddress | LensFollowerAddress => {
  ...
  if ((user as TalentProtocolRecommendedUser).follows?.followingOnTalentProtocol) {
    score += scoreMap.followingOnTalentProtocol;
  }
  if ((user as TalentProtocolRecommendedUser).follows?.followedOnTalentProtocol) {
    score += scoreMap.followedByTalentProtocol;
  }
